...
import org.hibernate.search.db.events.jpa.impl.JPAUpdateSource;
...
public class ManualUpdateIntegrationTest extends DatabaseIntegrationTest {
	...
	public void setup() throws SQLException {
		...
		for ( Class<?> indexRootType : Collections.singletonList( Place.class ) ) {
		...
		List<ExtendedTypeMetadata> extendedTypeMetadatas = metadataExtender.rehash(
				typeMetadatas,
				Collections.singletonList( Place.class )
		);
		Map<Class<?>, ExtendedTypeMetadata> extendedTypeMetadataPerIndexRoot = new HashMap<>();
		for ( ExtendedTypeMetadata extendedTypeMetadata : extendedTypeMetadatas ) {
			extendedTypeMetadataPerIndexRoot.put(
					extendedTypeMetadata.getOriginalTypeMetadata().getType(),
					extendedTypeMetadata
			);
		}
		this.containedInIndexOf = MetadataUtil.calculateInIndexOf( extendedTypeMetadatas, Collections.emptyList() );
